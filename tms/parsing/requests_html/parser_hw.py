...
class Parser(ABC):
    ...
    def __init__(self, url, pages=2, headers=None):
        self.url = url
        self.pages = pages
    ...
    def save_response(self, num=""):
        self.name = f'{self.url.split("://")[1].split("/")[0].replace(".", "_")}_{num}'
        with open(f'{self.file_path}{self.name}.html', "w") as file:
...
class ProxyParserHTML(Parser):
    ...
    def parce(self):
        ...
        for file_name, proxies in self.proxy_types.items():
            self.save_csv(file_name, proxies)
